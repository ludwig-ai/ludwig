...
def process_list_block(docstring, starting_point, section_end,
    ...
    docstring_slice = docstring[starting_point:None if section_end == -1 else section_end].replace(block, marker)
    ...
    for i in range(len(lines)):
        ...
        if line.startswith(':param '):
            ...
            lines[i] = '- __{}__ ({}):{}'.format(param_name, inside_brackets, description)
            ...
            if pos_open_bracket and pos_close_bracket:
                inside_brackets = ' ({})'.format(line[pos_open_bracket + 1:pos_close_bracket])
