...
class SequenceConcatCombiner(tf.keras.Model):
    def __init__(self, reduce_output=None, main_sequence_feature=None, **kwargs):
        super().__init__()
        logger.debug(" {}".format(self.name))
...
class SequenceCombiner(tf.keras.Model):
    def __init__(
        self, reduce_output=None, main_sequence_feature=None, encoder=None, **kwargs
    ):
        super().__init__()
        logger.debug(" {}".format(self.name))

        self.combiner = SequenceConcatCombiner(
            reduce_output=None, main_sequence_feature=main_sequence_feature
        )

        self.encoder_obj = get_from_registry(encoder, sequence_encoder_registry)(
            should_embed=False, reduce_output=reduce_output, **kwargs
        )
...
class ComparatorCombiner(tf.keras.Model):
    def __init__(
        ...
        **kwargs,
        ...
        self.bilinear_weights = tf.random.normal([fc_size, fc_size], dtype=tf.float32)
