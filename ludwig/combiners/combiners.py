...
class ComparatorCombiner(tf.keras.Model):
    def __init__(
        ...
        self.bilinear_weights = tf.Variable([fc_size, fc_size],
                                            dtype=tf.float32)
    ...
    def call(
        ...
        assert inputs.keys == self.required_inputs
        ...
        # Compare #
        ###########
        dot_product = tf.matmul(e1_hidden, e2_hidden)
        element_wise_mul = tf.math.multiply(e1_hidden, e2_hidden)
        abs_diff = tf.abs(e1_hidden - e2_hidden)
        bilinear_prod = tf.matmul(e1_hidden,
                                  tf.matmul(self.bilinear_weights, e2_hidden))
        hidden = concatenate(
            [dot_product, element_wise_mul, abs_diff, bilinear_prod],
            1
        )
