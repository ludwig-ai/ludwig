...
def get_combiner_conds():
    ...
    for combiner_type in combiner_types:
        ...
        combiner_schema = schema_utils.get_custom_schema_from_marshmallow_class(schema_cls)
...
class ConcatCombinerConfig(BaseCombinerConfig):
    ...
    dropout: float = schema_utils.FloatRange(default=0.0, min=0, max=1)
...
class SequenceCombinerConfig(BaseCombinerConfig):
    ...
    encoder: Optional[str] = schema_utils.StringOptions(list(sequence_encoder_registry.keys()))
    """TODO: Document parameters. (default: None)."""
...
class TabTransformerCombinerConfig(BaseCombinerConfig, CommonTransformerConfig):
    ...
    reduce_output: str = schema_utils.ReductionOptions(default="concat")
