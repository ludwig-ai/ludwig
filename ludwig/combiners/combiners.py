...
from ludwig.constants import BINARY, NAME, NUMERICAL, TYPE
...
class ConcatCombiner(tf.keras.Model):
    def __init__(
        ...
        **kwargs,
        ...
        if config.fc_layers is None and config.num_fc_layers is not None:
            fc_layers = []
...
class SequenceConcatCombiner(tf.keras.Model):
    def __init__(self, config: SequenceConcatCombinerConfig = None, **kwargs):
        super().__init__()
        logger.debug(f" {self.name}")
    ...
    def __call__(self, inputs, training=None, mask=None, **kwargs):  # encoder outputs
        if (
            self.main_sequence_feature is None
        ...
        hidden = tf.multiply(
            hidden, tf.cast(tf.expand_dims(sequence_mask, -1), dtype=tf.float32)
        )
...
class SequenceCombiner(tf.keras.Model):
    def __init__(self, config: SequenceCombinerConfig = None, **kwargs):
        super().__init__()
        logger.debug(f" {self.name}")
        self.combiner = SequenceConcatCombiner(config)

        self.encoder_obj = get_from_registry(config.encoder, sequence_encoder_registry)(
            should_embed=False, reduce_output=config.reduce_output, **kwargs
        )

        if (
            hasattr(self.encoder_obj, "supports_masking")
            and self.encoder_obj.supports_masking
        ):
            self.supports_masking = True

    def __call__(self, inputs, training=None, mask=None, **kwargs):  # encoder outputs
        # ================ Concat ================
        hidden = self.combiner(inputs, training=training, **kwargs)  # encoder outputs

        # ================ Sequence encoding ================
        hidden = self.encoder_obj(
            hidden["combiner_output"], training=training, **kwargs
        )
...
class TabNetCombiner(tf.keras.Model):
    def __init__(self, config: TabNetCombinerConfig = None, **kwargs):
        super().__init__()
        logger.debug(f" {self.name}")
    ...
    def build(self, input_shape):
        self.flatten_layers = {k: tf.keras.layers.Flatten() for k in input_shape.keys()}

    def call(self, inputs, training=None, mask=None, **kwargs):  # encoder outputs
        encoder_output_map = {k: inputs[k]["encoder_output"] for k in inputs}

        # ================ Flatten ================
        encoder_outputs = [
            self.flatten_layers[k](eo) for k, eo in encoder_output_map.items()
        ]
...
class TransformerCombiner(tf.keras.Model):
    ...
    def call(self, inputs, training=None, mask=None, **kwargs):  # encoder outputs
        ...
        # ================ Flatten ================
        batch_size = tf.shape(encoder_outputs[0])[0]
        encoder_outputs = [tf.reshape(eo, [batch_size, -1]) for eo in encoder_outputs]
        ...
        # ================ Transformer Layers ================
        hidden = self.transformer_stack(hidden, training=training, mask=mask)
...
class TabTransformerCombiner(tf.keras.Model):
    def __init__(
        ...
        **kwargs,
        ...
        if config.reduce_output is None:
            raise ValueError("TabTransformer requires the `resude_output` " "parametr")
        self.reduce_output = config.reduce_output
        self.reduce_sequence = SequenceReducer(reduce_mode=config.reduce_output)
        self.supports_masking = True
        ...
        if self.embed_input_feature_name:
            ...
            if self.embed_input_feature_name == "add":
                self.embed_i_f_name_layer = Embed(
                    vocab, config.hidden_size, force_embedding_size=True
                )
        ...
        logger.debug("  Projectors")
        self.projectors = [
            Dense(projector_size)
            for i_f in input_features
            if input_features[i_f].type != NUMERICAL
            and input_features[i_f].type != BINARY
        ]
        self.skip_features = [
            i_f
            for i_f in input_features
    ...
    def call(self, inputs, training=None, mask=None, **kwargs):  # encoder outputs
        skip_encoder_outputs = [
            inputs[k]["encoder_output"] for k in inputs if k in self.skip_features
        ]
        other_encoder_outputs = [
            inputs[k]["encoder_output"] for k in inputs if k not in self.skip_features
        ]
        ...
        projected = [
            self.projectors[i](eo) for i, eo in enumerate(other_encoder_outputs)
        ]
        hidden = tf.stack(projected)  # num_eo, bs, h
        hidden = tf.transpose(hidden, perm=[1, 0, 2])  # bs, num_eo, h
...
class ComparatorCombiner(tf.keras.Model):
    def __init__(
        ...
        **kwargs,
        ...
        self.bilinear_weights = tf.random.normal(
            [config.fc_size, config.fc_size], dtype=tf.float32
        )
    ...
    def call(self, inputs, training=None, mask=None, **kwargs):  # encoder outputs
        ...
        e1_enc_outputs = [tf.reshape(eo, [batch_size, -1]) for eo in e1_enc_outputs]
        ...
        e2_enc_outputs = [tf.reshape(eo, [batch_size, -1]) for eo in e2_enc_outputs]
        ...
        bilinear_prod = tf.matmul(
            e1_hidden, tf.matmul(self.bilinear_weights, tf.transpose(e2_hidden))
        )
        hidden = concatenate(
