...
class LudwigModel:
    ...
    def __init__(
        ...
        # setup TensorFlow
        self.backend.initialize_tensorflow(
            gpus=gpus,
        ...
        with upload_output_directory(output_directory) as (output_directory, upload_fn):
            ...
            with self.backend.create_trainer(
                **self.config[TRAINING],
                ...
                if (
                    self.config[TRAINING][BATCH_SIZE] == AUTO
                    or self.config[TRAINING][EVAL_BATCH_SIZE] == AUTO
                ):
                    # TODO (ASN): add support for substitute_with_max parameter
                    tuned_batch_size = trainer.tune_batch_size(
                        self.config, training_set, random_seed=random_seed
                    )
                ...
                try:
                    ...
                    if self.backend.is_coordinator() and validation_set is not None:
                        ...
                        logger.info(
                            "Best validation model epoch: {}".format(
                        ...
                        if test_set is not None:
                            best_vali_metric_epoch_test_metric = train_testset_stats[
                                validation_field
                            ][validation_metric][epoch_best_vali_metric]
    ...
    def train_online(
        ...
        if not self._online_trainer:
            self._online_trainer = self.backend.create_trainer(
                **self.config[TRAINING], random_seed=random_seed, debug=debug
            )
    ...
    def experiment(
        ...
        **kwargs,
        ...
        if eval_split == TRAINING:
            ...
            logger.warning(
                f"Eval split {eval_split} not supported. "
        ...
        if eval_set is not None:
            ...
            logger.warning(
                f"The evaluation set {eval_set} was not provided. "
...
def kfold_cross_validate(
    ...
    **kwargs,
    ...
    data_df = load_dataset(
        dataset, data_format=data_format, df_lib=backend.df_engine.df_lib
    )

    kfold_cv_stats = {}
    kfold_split_indices = {}

    for train_indices, test_indices, fold_num in generate_kfold_splits(
        data_df, num_folds, random_seed
    ):
...
def get_experiment_description(
    ...
    try:
        with open(os.devnull, "w") as devnull:
            is_a_git_repo = (
                subprocess.call(
                    ["git", "branch"], stderr=subprocess.STDOUT, stdout=devnull
                )
                == 0
            )
        if is_a_git_repo:
            description["commit_hash"] = subprocess.check_output(
                ["git", "rev-parse", "HEAD"]
            ).decode("utf-8")[:12]
    except:
        pass
