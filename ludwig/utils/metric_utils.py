...
def masked_correct_predictions(targets: Tensor, preds: Tensor,
                               targets_sequence_lengths: Tensor) -> Tensor:
    """
    Params:
        targets: 2D tensor
        preds: 2D tensor
    ...
    padded_truncated_preds = F.pad(truncated_preds,
                                   pad=[0, targets.shape[1] - truncated_preds.shape[1]])
    correct_preds = padded_truncated_preds == targets

    mask = sequence_mask(lengths=targets_sequence_lengths,
                         maxlen=correct_preds.shape[1],
                         dtype=torch.int32)
    _, masked_correct_preds = dynamic_partition(data=correct_preds,
                                                partitions=mask,
                                                num_partitions=2)
