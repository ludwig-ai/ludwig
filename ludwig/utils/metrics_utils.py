...
class ConfusionMatrix:
        ...
        if labels is not None:
            ...
            self.label2idx = {
                ...
                for idx, label in enumerate(
                    np.unique([self.predictions, self.conditions])
                )
            }
            self.idx2label = {
                idx: str(label)
    ...
    def true_negatives(self, idx):
        return (
            self.all
            - self.sum_predictions[idx]
            - self.sum_conditions[idx]
            + self.true_positives(idx)
        )
    ...
    def markedness(self, idx):
        return (
            self.positive_predictive_value(idx)
            + self.negative_predictive_value(idx)
            - 1
        )
    ...
    def avg_precision(self, average="macro"):
        return metrics.precision_score(
            self.conditions, self.predictions, average=average
        )
    ...
    def avg_fbeta_score(self, beta, average="macro"):
        return metrics.fbeta_score(
            self.conditions, self.predictions, beta=beta, average=average
        )
...
def roc_curve(conditions, prediction_scores, pos_label=None, sample_weight=None):
    return metrics.roc_curve(conditions, prediction_scores, pos_label, sample_weight)


def roc_auc_score(conditions, prediction_scores, average="micro", sample_weight=None):
    try:
        return metrics.roc_auc_score(
            conditions, prediction_scores, average, sample_weight
        )
...
def precision_recall_curve(
    conditions, prediction_scores, pos_label=None, sample_weight=None
):
    return metrics.precision_recall_curve(
        conditions, prediction_scores, pos_label, sample_weight
    )


def average_precision_score(
    conditions, prediction_scores, average="micro", sample_weight=None
):
    # average == [micro, macro, sampled, weidhted]
    return metrics.average_precision_score(
        conditions, prediction_scores, average=average, sample_weight=sample_weight
    )
