...
default_optimizer_params_registry = {
    ...
    "nadam": {},
    "rmsprop": {"decay": 0.9, "momentum": 0.0, "epsilon": 1e-10, "centered": False},
}
default_optimizer_params_registry[
    "stochastic_gradient_descent"
...
def _perform_sanity_checks(config):
    ...
    assert (
        len(config["input_features"]) > 0
    ), "config needs to have at least one input feature"

    assert (
        len(config["output_features"]) > 0
...
def _merge_hyperopt_with_training(config: dict) -> None:
    ...
    if (
        ...
        if time_attr == "time_total_s":
            training[
                "epochs"
            ] = sys.maxsize  # essentially continue training until stopped
        else:
...
def merge_with_defaults(config):
    ...
    config["preprocessing"] = merge_dict(
        default_preprocessing_parameters, config.get("preprocessing", {})
    )
    ...
    if stratify is not None:
        ...
        elif [f for f in features if f[COLUMN] == stratify][0][TYPE] not in {
            BINARY,
            CATEGORY,
        }:
    ...
    for input_feature in config["input_features"]:
        get_from_registry(input_feature[TYPE], input_type_registry).populate_defaults(
            input_feature
        )
    ...
    for output_feature in config["output_features"]:
        get_from_registry(output_feature[TYPE], output_type_registry).populate_defaults(
            output_feature
        )
