...
def get_fbank(
    ...
    lower_limit_mel = 0
    list_mel_points = np.linspace(
        lower_limit_mel, upper_limit_mel, num_filter_bands + 2
    )
    mel_fbank_matrix = _get_mel_fbank_matrix(
...
def _get_mel_fbank_matrix(
    ...
    mel_scaled_fbank = np.zeros(
        (num_filter_bands, num_ess_fft_points), dtype=np.float32
    )
...
def _preprocess_to_padded_matrix(
    data, window_length_in_samp, window_shift_in_samp, zero_mean_offset=False
):
    num_input = data.shape[0]
    num_output = get_num_output_padded_to_fit_input(
    ...
    for num_output_idx in range(num_output):
        ...
        end_padded_idx = (
            window_length_in_samp if not is_last_output else end_idx - start_idx
        )
...
def get_num_output_padded_to_fit_input(
    num_input, window_length_in_samp, window_shift_in_samp
):
...
def get_max_length_stft_based(
    length_in_samp, window_length_in_s, window_shift_in_s, sampling_rate_in_hz
):
    window_length_in_samp = get_length_in_samp(window_length_in_s, sampling_rate_in_hz)
    window_shift_in_samp = get_length_in_samp(window_shift_in_s, sampling_rate_in_hz)
    return get_num_output_padded_to_fit_input(
        length_in_samp, window_length_in_samp, window_shift_in_samp
    )
...
def calculate_var(sum1, sum2, count):
    return (
        (sum2 - ((sum1 * sum1) / float(count))) / float(count - 1) if count > 1 else 0.0
    )
