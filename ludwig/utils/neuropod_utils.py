...
def postprocess_for_neuropod(predicted, config):
    ...
    for output_feature in config["output_features"]:
        ...
        if feature_type == BINARY:
            ...
            postprocessed[feature_name + "_probabilities"] = predicted[feature_name][
                PROBABILITIES
            ].astype("float64")
            ...
            predictions = list(
                map(lambda x: " ".join(x), predicted[feature_name][PREDICTIONS])
            )
            postprocessed[feature_name + "_predictions"] = np.expand_dims(
            ...
            postprocessed[feature_name + "_predictions"] = predicted[feature_name][
                PREDICTIONS
            ].astype("float64")
        else:
            postprocessed[feature_name + "_predictions"] = np.expand_dims(
