...
def remove_left_padding(input_ids: torch.Tensor, tokenizer: PreTrainedTokenizer):
    """Removes left padding and other tokens until the first BOS token from the input_ids tensor."""
    # Remove all PAD tokens
    pad_idxs = torch.where(input_ids == tokenizer.pad_token_id)[0]  # all PAD token locations
    input_ids_no_padding = input_ids
    if len(pad_idxs) != 0:
        pad_idx = pad_idxs[-1]  # get last PAD token location
        input_ids_no_padding = input_ids[pad_idx + 1 :]
