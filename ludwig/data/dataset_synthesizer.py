...
def generate_audio(feature):
    ...
    audio = (
        np.sin(np.arange(num_samples) / 100 * 2 * np.pi)
        * 2
    ...
    try:
        ...
        raise OSError(
            "Unable to create a folder for audio or save audio to disk." "{}".format(e)
        )
...
def generate_image(feature):
    ...
    try:
        ...
        raise OSError(
            "Unable to create a folder for images/save image to disk." "{}".format(e)
        )
...
def generate_vector(feature):
    ...
    return " ".join(
        [str(100 * random.random()) for _ in range(feature.get("vector_size", 10))]
    )
...
def cli_synthesize_dataset(
    dataset_size: int, features: List[dict], output_path: str, **kwargs
) -> None:
    """Symthesizes a dataset for testing purposes.
...
def cli(sys_argv):
    ...
    parser.add_argument(
        "-d", "--dataset_size", help="size of the dataset", type=int, default=100
    )
    parser.add_argument(
        "-f",
        "--features",
