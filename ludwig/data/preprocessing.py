...
from ludwig.features.feature_registries import base_type_registry, input_type_registry
...
class DataFormatPreprocessor(ABC):
    ...
    def preprocess_for_prediction(
        dataset, features, preprocessing_params, training_set_metadata, backend
    ):
        pass
...
class ParquetPreprocessor(DataFormatPreprocessor):
    ...
    def prepare_processed_data(
        ...
        validation_set = (
            validation_set if validation_set and path_exists(validation_set) else None
        )
...
class HDF5Preprocessor(DataFormatPreprocessor):
    ...
    def prepare_processed_data(
        ...
        if not training_set_metadata:
            raise ValueError(
                "When providing HDF5 data, " "training_set_metadata must not be None."
            )
        ...
        if dataset is not None:
            training_set, test_set, validation_set = load_hdf5(
                dataset, features, shuffle_training=True
            )
            ...
            if validation_set is not None:
                validation_set = load_hdf5(
                    validation_set, shuffle_training=False, **kwargs
                )
...
def build_dataset(
    ...
    dataset_cols = cast_columns(
        dataset_df, proc_features, global_preprocessing_parameters, backend
    )

    logger.debug("build metadata")
    metadata = build_metadata(
        metadata, dataset_cols, proc_features, global_preprocessing_parameters, backend
    )

    logger.debug("build data")
    proc_cols = build_data(
        dataset_cols, proc_features, metadata, backend, skip_save_processed_input
    )
...
def build_metadata(
    ...
    for feature in features:
        ...
        fill_value = precompute_fill_value(
            dataset_cols, feature, preprocessing_parameters, backend
        )
        ...
        get_feature_meta = get_from_registry(
            feature[TYPE], base_type_registry
        ).get_feature_meta
...
def build_data(
    input_cols, features, training_set_metadata, backend, skip_save_processed_input
):
    proc_cols = {}
    for feature in features:
...
def preprocess_for_training(
    ...
    with file_lock(lock_path, lock_file=".lock_preprocessing"):
        ...
        data_format_processor = get_from_registry(
            data_format, data_format_preprocessor_registry
        )
        ...
        if cached or data_format == "hdf5":
            ...
            replace_text_feature_level(
                features, [training_set, validation_set, test_set]
            )
        ...
        if test_set is not None:
            ...
            test_dataset = backend.dataset_manager.create(
                test_set, config, training_set_metadata
            )
...
def _preprocess_file_for_training(
    ...
    if dataset:
        ...
        if (
            backend.is_coordinator()
            and not skip_save_processed_input
        ...
        concatenated_df = concatenate_files(
            training_set, validation_set, test_set, read_fn, backend
        )
...
def preprocess_for_prediction(
    ...
    preprocessing_params = merge_dict(
        default_preprocessing_parameters, config[PREPROCESSING]
    )
            ...
            for dataset in datasets:
                if dataset is not None:
                    dataset[feature[PROC_COLUMN]] = dataset[
                        "{}_{}".format(feature[PROC_COLUMN], feature["level"])
                    ]
                    for level in ("word", "char"):
                        name_level = "{}_{}".format(feature[PROC_COLUMN], level)
