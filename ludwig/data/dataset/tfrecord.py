...
from ludwig.utils.data_utils import DATA_TRAIN_HDF5_FP, load_json
...
class TFRecordDataset(Dataset):
    ...
    def initialize_batcher(
        ...
        path = os.path.join(self.url, f"*.tfrecords{compression_ext}")
        ...
        dataset = files.interleave(
            lambda x: tf.data.TFRecordDataset(
                x, compression_type=self.compression_type
            ),
        ...
        def parse_sequence(feats, feat_lists):
            def parse(binary):
                context, sequence = tf.io.parse_single_sequence_example(
                    binary, context_features=feats, sequence_features=feat_lists
                )
...
class TFRecordDatasetManager:
    ...
    def create(self, dataset, config, training_set_metadata):
        ...
        return TFRecordDataset(dataset, features, training_set_metadata)
