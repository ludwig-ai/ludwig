...
def calculate_checksum(original_dataset: CacheableDataset, config: ModelConfigDict):
    ...
    The checksum is used to determine if the dataset and model config have changed since the last time the
    model was trained. If either has changed, a different checksum will be produced which will lead to a cache
    miss and force preprocessing to be performed again.
    """
    ...
    info = {
        ...
        "feature_proc_columns": set([feature[PROC_COLUMN] for feature in features]),
