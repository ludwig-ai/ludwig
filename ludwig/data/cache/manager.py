...
class DatasetCache:
    ...
    def get(self):
        ...
        cached_training_set = (
            self.cache_map[TRAINING] if path_exists(self.cache_map[TRAINING]) else None
        )

        cached_test_set = (
            self.cache_map[TEST] if path_exists(self.cache_map[TEST]) else None
        )

        cached_validation_set = (
            self.cache_map[VALIDATION]
            if path_exists(self.cache_map[VALIDATION])
            else None
        )

        valid = (
            self.checksum == cache_training_set_metadata.get(CHECKSUM)
            and cached_training_set is not None
        )
