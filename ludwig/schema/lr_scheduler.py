...
class LRSchedulerConfig(schema_utils.BaseMarshmallowConfig, ABC):
    ...
    T_mult: int = schema_utils.PositiveInteger(
        default=1,
        description="Period multiplier after each restart for cosine annealing decay. Default: 1, i.e.,"
        " restart at fixed periods. If set to a value larger, the period between restarts increases by that"
    ...
    eta_min: float = schema_utils.FloatRange(
        ...
        min=0,
        max=1,
        description="Minimum learning rate for cosine annealing decay. Default: 0.",
