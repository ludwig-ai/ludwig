...
from globals import REPORT_JSON, CONFIG_YAML, EXPERIMENT_RUN
from ludwig.globals import MODEL_HYPERPARAMETERS_FILE_NAME
from ludwig.utils.fs_utils import get_fs_and_path
from ludwig.utils.defaults import merge_with_defaults, default_random_seed
...
def load_from_module(dataset_module: ModuleType, output_feature: Dict[str, str],
                     subsample_frac: float = 1) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Take in Ludwig Python module of the dataset, load it,
    and split it if it isn't already (by adding "split" column).
    ...
    if output_feature["type"] == CATEGORY:
        dataset = get_repeatable_train_val_test_split(dataset, stratify_colname=output_feature["name"],
                                                      random_seed=default_random_seed, frac_train=0.75,
                                                      frac_val=0.1, frac_test=0.15)
    else:
        dataset = get_repeatable_train_val_test_split(dataset, random_seed=default_random_seed, frac_train=0.75,
                                                      frac_val=0.1, frac_test=0.15)
...
def export_artifacts(experiment: Dict[str, str], report_path: str,
                     experiment_output_directory: str, export_base_path: str) -> None:
    ...
    try:
        experiment_name = experiment["experiment_name"] if experiment["experiment_name"] else datetime.now().strftime(
            "%Y.%m.%d.%H:%M:%S")
        ...
        fs.put(report_path,
               ...
               recursive=True)
        fs.put(os.path.join("configs", experiment["config_path"]),
               os.path.join(export_full_path, CONFIG_YAML),
               recursive=True)
        fs.put(os.path.join(experiment["dataset_name"], EXPERIMENT_RUN, "model", MODEL_HYPERPARAMETERS_FILE_NAME),
        ...
        try:
            ...
            logging.error("Couldn't export '{}' to bucket".format(experiment_output_directory))
            logging.error(e)
