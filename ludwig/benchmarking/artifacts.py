import os
from ludwig.utils.data_utils import load_yaml, load_json
...
class BenchmarkingArtifact:
    def __init__(self, benchmarking_config: dict, experiment_idx: int):
        ...
        if self.experiment_config["process_config_file_path"]:
            with open(self.experiment_config["process_config_file_path"], "r") as f:
        ...
        self.model_hyperparameters = load_json(os.path.join(self.experiment_run_path, "model", "model_hyperparameters.json"))
        self.training_progress = load_json(os.path.join(self.experiment_run_path, "model", "training_progress.json"))
        self.training_set_metadata = load_json(os.path.join(self.experiment_run_path, "model", "training_set_metadata.json"))
