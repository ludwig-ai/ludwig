...
class ECD(LudwigModule):
    ...
    def __init__(
        ...
        # ================ Combiner ================
        logger.debug(f'Combiner {combiner_def[constants.TYPE]}')
        combiner_class = get_combiner_class(combiner_def[constants.TYPE])
    ...
    def forward(self,
        ...
        for output_feature_name, decoder in self.output_features.items():
            ...
            for decoder_output_name, tensor in decoder_outputs.items():
                output_feature_utils.set_output_feature_tensor(output_logits, output_feature_name,
                                                               decoder_output_name, tensor)
    ...
    def predictions(self, inputs, output_features=None):
        ...
        if output_features is None:
            ...
            if output_features == 'all':
                ...
                raise ValueError("'output_features' {} is not a valid for this model. "
                                 "Available ones are: {}".format(output_features,
                                                                 set(self.output_features.keys())))
        elif isinstance(output_features, list or set):
            if output_features.issubset(self.output_features):
                of_list = output_features
            else:
                raise ValueError("'output_features' {} must be a subset of "
                                 "available features {}".format(output_features,
                                                                set(self.output_features.keys())))
        else:
            raise ValueError("'output_features' must be None or a string or a list "
                             "of output features")
    ...
    def get_metrics(self):
        ...
        all_of_metrics[constants.COMBINED] = {
            constants.LOSS: get_scalar_from_ludwig_metric(self.eval_loss_metric)
        }
    ...
    def collect_weights(self, tensor_names=None, **kwargs):
        ...
        return [
            named_param for named_param in self.named_parameters() if named_param[0] in tensor_set
        ]

    def get_args(self):
        return (self._input_features_df, self._combiner_def, self._output_features_df,
                self._random_seed)
...
def build_inputs(input_features_def: List[Dict[str, Any]], **kwargs) -> Dict[str, InputFeature]:
    ...
    for input_feature_def in input_features_def:
        input_features[input_feature_def[constants.NAME]] = build_single_input(
            input_feature_def, input_features, **kwargs)
    return input_features


def build_single_input(input_feature_def: Dict[str, Any],
                       other_input_features: Dict[str, InputFeature], **kwargs) -> InputFeature:
    logger.debug(
        f'Input {input_feature_def[constants.TYPE]} feature {input_feature_def[constants.NAME]}')
    ...
    input_feature_class = get_from_registry(input_feature_def[constants.TYPE], input_type_registry)
...
def build_outputs(output_features_def, combiner, **kwargs):
    ...
    for output_feature_def in output_features_def:
        ...
        output_feature = build_single_output(output_feature_def, combiner, output_features,
                                             **kwargs)
        output_features[output_feature_def[constants.NAME]] = output_feature
...
def build_single_output(output_feature_def, feature_hidden, other_output_features, **kwargs):
    logger.debug(
        f'Output {output_feature_def[constants.TYPE]} feature {output_feature_def[constants.NAME]}')

    output_feature_class = get_from_registry(output_feature_def[constants.TYPE],
                                             output_type_registry)
    output_feature_obj = output_feature_class(output_feature_def)
    # weighted_train_mean_loss, weighted_eval_loss, output_tensors = \
    #   output_feature_obj.concat_dependencies_and_build_output(
    #    feature_hidden,
