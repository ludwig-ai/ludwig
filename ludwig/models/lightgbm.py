...
from ludwig.constants import BINARY, CATEGORY, COMBINED, LOSS, NUMERICAL
...
class LightGBMTrainer(Trainer):
    def train(self, model, training_set, validation_set=None, test_set=None, save_path="model", **kwargs):
        ...
        categorical_features = [
            i
            for i, feature in enumerate(model.input_features.values())
            if feature.type == CATEGORY or feature.type == BINARY
        ]

        # TODO: update training metrics
        (train_metrics, vali_metrics, test_metrics) = self.initialize_training_metrics(model.output_features)
