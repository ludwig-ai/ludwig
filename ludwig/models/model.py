...
import os
import re
# os.environ['TF_CPP_MIN_VLOG_LEVEL']='3'  # enables a lot of additional logging
import signal
import sys
import time
...
from ludwig.globals import MODEL_WEIGHTS_FILE_NAME, \
    ...
    MODEL_WEIGHTS_PROGRESS_FILE_NAME, get_disable_progressbar, get_on_master
...
class Model:
    ...
    def __init__(self, input_features, output_features, combiner, training,
        ...
        self.session = None
        self.__build(input_features, output_features, combiner,
                     training, preprocessing, random_seed, **kwargs)

    def __build(self, input_features, output_features, combiner, training,
                preprocessing, random_seed, **kwargs):
        ...
        with graph.as_default():
            ...
            self.regularization_lambda = tf.placeholder(
                tf.float32, name='regularization_lambda')
            ...
            # ================ Inputs ================
            feature_encodings = build_inputs(input_features,
                                             self.regularizer,
                                             self.dropout_rate,
            ...
            # ================ Outputs ================
            outs = build_outputs(output_features, hidden,
                                 hidden_size,
                                 regularizer=self.regularizer,
                                 dropout_rate=self.dropout_rate,
                                 is_training=self.is_training)
            self.train_reg_mean_loss, self.eval_combined_loss, self.regularization_loss, output_tensors = outs
    ...
    def feed_dict(self, batch, regularization_lambda=0.0, learning_rate=0.001,
                  dropout_rate=0.0, is_training=True):
    ...
    def train(self, training_set, validation_set=None, test_set=None,
              validation_field=None, validation_measure=None,
              save_path='model',
              regularization_lambda=0.0, epochs=100, learning_rate=0.001,
              batch_size=128, bucketing_field=None,
              dropout_rate=0.0, early_stop=20,
        ...
        :param validation_field: The first output feature, by default it is set as the same field of the first output feature.
        :param validation_measure: Measure used on the validation field, it is accuracy by default
        ...
        :param bucketing_field:when batching, buckets datapoints based the length of a field together.
         Bucketing on text length speeds up training of RNNs consistently, 30% in some cases
        :type bucketing_field:
        :param dropout_rate: dropout_rate probability (probability of dropping a neuron in a given layer)
        :type dropout_rate: Float
        :param early_stop: How many epochs without any improvement in the validation_measure triggers the algorithm to stop
        :type early_stop: Integer
        :param reduce_learning_rate_on_plateau: Reduces the learning rate when the algorithm hits a plateau
        (i.e. the performance on the validation doesn't improve)
        ...
        :param increase_batch_size_on_plateau: Increase the batch size on a plateau
        ...
        :param increase_batch_size_on_plateau_rate: The rate at which the batch size increases.
        ...
        model_weights_progress_path = os.path.join(
            save_path, MODEL_WEIGHTS_PROGRESS_FILE_NAME)
    ...
    def evaluation(self, session, dataset, dataset_name, regularization_lambda,
                   stats, tables,
                   batch_size=128, bucketing_field=None):
        ...
        tables['combined'].append([dataset_name,
                                   results['combined'][LOSS],
                                   results['combined'][ACCURACY]])
        return stats, tables

    def batch_evaluation(self, session, dataset, batch_size,
                         bucketing_field=None, regularization_lambda=0.0,
                         is_training=False,
                         collect_predictions=False, only_predictions=False,
                         name=None):
        output_nodes = self.get_output_nodes(collect_predictions,
                                             only_predictions)
        ...
        if 'combined' in output_stats and LOSS in output_stats['combined']:
            regularization = session.run([self.regularization_loss],
                                         feed_dict={
                                             self.regularization_lambda: regularization_lambda
                                         }
                                         )[0]
        ...
        for output_feature in output_features:
            ...
            for stat in output_config:
                ...
                if (output_type == PREDICTION and (
                    ...
                    output_nodes[field_name][output_name] = \
                        getattr(self, output_name + '_' + field_name)

        if not only_predictions:
            output_nodes['eval_combined_loss'] = getattr(self,
                                                         'eval_combined_loss')
    ...
    def update_output_stats_batch(self, output_stats, seq_set_size,
                                  collect_predictions, only_predictions,
                                  result):
        output_features = self.hyperparameters['output_features']
    ...
    def update_output_stats(self, output_stats, set_size, seq_set_size,
                            collect_predictions, only_predictions):
        output_features = self.hyperparameters['output_features']
    ...
    def check_progress_on_validation(self, progress_tracker, validation_field,
                                     validation_measure,
                                     session, model_weights_path,
                                     model_hyperparameters_path,
                                     reduce_learning_rate_on_plateau,
        ...
        if improved(progress_tracker.vali_stats[validation_field][
    ...
    def predict(self, dataset, batch_size, only_predictions=False, gpus=None,
                gpu_fraction=1, **kwargs):
        if self.session is None:
            session = self.initialize_session(gpus, gpu_fraction)
        ...
        # predict
        predict_stats = self.batch_evaluation(session, dataset, batch_size,
                                              is_training=False,
    ...
    def collect_activations(self, dataset, tensor_names, batch_size, gpus=None,
                            gpu_fraction=1, **kwargs):
        if self.session is None:
            session = self.initialize_session(gpus, gpu_fraction)
        ...
        for tensor_name in tensor_names:
            if tensor_name not in opration_names:
                raise ValueError(
                    'Tensor / operation {} not present in the model graph'.format(
                        tensor_name))

        # collect tensors
        collected_tensors = self.batch_collect_activations(
            session, dataset, batch_size, tensor_names)

        return collected_tensors

    def collect_weights(self, tensor_names, gpus=None, gpu_fraction=1,
                        **kwargs):
        if self.session is None:
            session = self.initialize_session(gpus, gpu_fraction)
    ...
    def save_hyperparameters(self, hyperparameters, save_path):
        ...
        # because the weights are already saved in the model, no need to reload from their path
        # when loading the model next time
    ...
    def initialize_batcher(self, dataset, batch_size=128, bucketing_field=None,
        ...
        if bucketing_field is not None:
            ...
            batcher = BucketedBatcher(dataset, bucketing_field=bucketing_field,
                                      batch_size=batch_size, buckets=10,
                                      ignore_last=ignore_last,
                                      should_shuffle=should_shuffle,
                                      should_trim=should_trim,
                                      trim_side=trim_side)
        else:
            batcher = Batcher(dataset, batch_size,
                              should_shuffle=should_shuffle,
                              ignore_last=ignore_last)
        return batcher

    def resume_session(self, session, save_path, model_weights_path,
                       model_weights_progress_path):
...
class ProgressTracker:
    def __init__(self, epoch, batch_size, steps, last_improvement_epoch,
                 best_valid_measure, learning_rate, num_reductions_lr,
