...
class BasePredictor(ABC):
    ...
    def batch_evaluation(
        self, model, dataset, collect_predictions=False, dataset_name=None
    ):
        raise NotImplementedError()

    @abstractmethod
    def batch_collect_activations(
        self, model, layer_names, dataset, bucketing_field=None
    ):
        raise NotImplementedError()
...
class Predictor(BasePredictor):
    ...
    def __init__(self, batch_size=128, horovod=None, debug=False, **kwargs):
    ...
    def batch_predict(
        ...
        with dataset.initialize_batcher(
            self._batch_size, should_shuffle=False, horovod=self._horovod
        ) as batcher:
            ...
            if self.is_coordinator():
                progress_bar = tqdm(
                    ...
                    else f"Prediction {dataset_name: <5.5}",
        ...
        with dataset.initialize_batcher(
            ...
            if self.is_coordinator():
                progress_bar = tqdm(
                    desc="Evaluation"
                    if dataset_name is None
                    else f"Evaluation {dataset_name: <5.5}",
                    total=batcher.steps_per_epoch,
                    file=sys.stdout,
                    disable=is_progressbar_disabled(),
                )
    ...
    def batch_collect_activations(
        ...
        keras_model = model.get_connected_model(
            inputs=keras_model_inputs, training=False
        )
        ...
        activation_model = tf.keras.Model(
            inputs=keras_model_inputs, outputs=output_nodes
        )

        with dataset.initialize_batcher(
            self._batch_size, should_shuffle=False
        ) as batcher:
            ...
            while not batcher.last_batch():
                ...
                for layer_name, output in outputs.items():
                    ...
                    if isinstance(output, tf.Tensor):
                        ...
                        output = [
                            (f"_{idx}", tensor) for idx, tensor in enumerate(output)
                        ]
    ...
    def merge_workers_metrics(self, metrics):
        ...
        merged_output_metrics = sum_dicts(
            all_workers_output_metrics, dict_type=OrderedDict
        )
...
def calculate_overall_stats(
    output_features, predictions, dataset, training_set_metadata
):
    overall_stats = {}
