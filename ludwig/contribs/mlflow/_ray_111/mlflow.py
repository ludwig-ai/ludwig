...
class MLflowLoggerUtil(_MLflowLoggerUtil):
    def _start_active_run(self, run_name: Optional[str] = None, tags: Optional[Dict] = None):
...
class MLflowLoggerCallback(LoggerCallback):
    ...
    .. code-block:: python
        ...
        tune.run(
            ...
            callbacks=[MLflowLoggerCallback(experiment_name="experiment1", tags=tags, save_artifact=True)],
        )
    """
...
class MLflowLogger(Logger):
    ...
    Requires the experiment configuration to have a MLflow Experiment ID or manually set the proper environment
    variables.
    """

    def _init(self):
        raise DeprecationWarning(
            "The legacy MLflowLogger has been " "deprecated. Use the MLflowLoggerCallback " "instead."
        )
    ...
    .. code-block:: python
        ...
        tune.run(
            ...
            config={
                ...
                "mlflow": {"experiment_name": "my_experiment", "tracking_uri": mlflow.get_tracking_uri()},
            },
        )
    """
    if ray.util.client.ray.is_connected():
