...
class SST(ABC, ZipDownloadMixin, MultifileJoinProcessMixin, CSVLoadMixin, BaseDataset):
    ...
    def process_downloaded_dataset(self):
        sentences_df = pd.read_csv(
            os.path.join(
                self.raw_dataset_path, "stanfordSentimentTreebank/datasetSentences.txt"
            ),
            sep="\t",
        )
        ...
        datasplit_df = pd.read_csv(
            os.path.join(
                self.raw_dataset_path, "stanfordSentimentTreebank/datasetSplit.txt"
            ),
            sep=",",
        )
        ...
        with open(
            os.path.join(
                self.raw_dataset_path, "stanfordSentimentTreebank/dictionary.txt"
            )
        ) as f:
            Lines = f.readlines()
            for line in Lines:
        ...
        with open(
            os.path.join(
                self.raw_dataset_path, "stanfordSentimentTreebank/sentiment_labels.txt"
            )
        ) as f:
            Lines = f.readlines()
            for line in Lines:
        ...
        if self.include_subtrees:
            ...
            with open(
                os.path.join(
                    self.raw_dataset_path, "stanfordSentimentTreebank/STree.txt"
                )
            ) as f:
                Lines = f.readlines()
                for line in Lines:
            ...
            with open(
                os.path.join(
                    self.raw_dataset_path, "stanfordSentimentTreebank/SOStr.txt"
                )
            ) as f:
                Lines = f.readlines()
                for line in Lines:
        ...
        for split_name, split_id in splits.items():
            ...
            if split_name == "train" and self.include_subtrees:
                ...
                for sentence_idx in sentence_idcs:
                    ...
                    sentence_idx -= 1
                    subtrees = sentence_subtrees(
                        sentence_idx, trees_pointers, trees_phrases
                    )

                    sentence_idx += 1
                    sentence_phrase = list(
                        sentences_df[sentences_df["sentence_index"] == sentence_idx][
                            "sentence"
                        ]
                    )[0]
                    ...
                    label = self.get_sentiment_label(
                        id2sent, phrase2id[sentence_phrase]
                    )
            ...
            final_csv.to_csv(
                os.path.join(self.raw_dataset_path, f"{split_name}.csv"), index=False
            )
...
def format_text(text: str):
    ...
    return " ".join(
        [w.encode("latin1").decode("utf-8") for w in text.strip().split(" ")]
    )
