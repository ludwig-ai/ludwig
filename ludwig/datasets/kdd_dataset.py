...
class KDDCup2009Dataset(
    ...
    def process_downloaded_dataset(self, header=0):
        ...
        targets = (
            pd.read_csv(
                os.path.join(
                    self.raw_dataset_path, f"orange_small_train_{self.task_name}.labels"
                ),
                header=None,
            )[0]
            .astype(str)
        ...
        train_idcs = pd.read_csv(
            os.path.join(
                self.raw_dataset_path, f"stratified_train_idx_{self.task_name}.txt"
            ),
            header=None,
        )[0]
        ...
        val_idcs = pd.read_csv(
            os.path.join(
                self.raw_dataset_path, f"stratified_test_idx_{self.task_name}.txt"
            ),
            header=None,
        )[0]
        ...
        df.to_csv(
            os.path.join(self.processed_temp_path, self.csv_filename), index=False
        )
