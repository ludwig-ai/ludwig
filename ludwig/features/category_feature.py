...
class CategoryOutputFeature(CategoryFeatureMixin, OutputFeature):
    ...
    def predictions(self, inputs, **kwargs):  # logits
        ...
        probabilities = tf.nn.softmax(logits, name=f"probabilities_{self.feature_name}")

        predictions = tf.argmax(logits, -1, name=f"predictions_{self.feature_name}")
    ...
    def _setup_metrics(self):
        ...
        self.metric_functions[ACCURACY] = CategoryAccuracy(name="metric_accuracy")
    ...
    def update_config_with_metadata(output_feature, feature_metadata, *args, **kwargs):
        ...
        output_feature["top_k"] = min(
            output_feature["num_classes"], output_feature["top_k"]
        )
            ...
            if "class_similarities" in output_feature[LOSS]:
                ...
                for i in range(len(similarities)):
                    similarities[i, :] = softmax(
                        similarities[i, :], temperature=temperature
                    )
    ...
    def calculate_overall_stats(predictions, targets, train_set_metadata):
        ...
        confusion_matrix = ConfusionMatrix(
            targets, predictions[PREDICTIONS], labels=train_set_metadata["idx2str"]
        )
