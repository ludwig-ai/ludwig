...
from ludwig.utils.strings_utils import UNKNOWN_SYMBOL, tokenizer_registry
...
def should_regularize(regularize_layers):
    ...
    if isinstance(regularize_layers, bool) and regularize_layers:
        ...
        isinstance(regularize_layers, (list, tuple))
        and regularize_layers
        and regularize_layers[-1]
    ):
        regularize = True
    return regularize
...
def set_str_to_idx(set_string, feature_dict, tokenizer_name):
    ...
    out = [
        feature_dict.get(item, feature_dict[UNKNOWN_SYMBOL])
        for item in tokenizer(set_string)
    ]
