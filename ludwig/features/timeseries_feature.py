...
class TimeseriesFeatureMixin:
    ...
    def get_feature_meta(column, preprocessing_parameters, backend):
        ...
        max_length = min(
            preprocessing_parameters["timeseries_length_limit"], max_length
        )
    ...
    @staticmethod
    def build_matrix(
        timeseries, tokenizer_name, length_limit, padding_value, padding, backend
    ):
...
class TimeseriesInputFeature(TimeseriesFeatureMixin, SequenceInputFeature):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        assert (
            inputs.dtype == tf.float16
            or inputs.dtype == tf.float32
            or inputs.dtype == tf.float64
        )
        assert len(inputs.shape) == 2

        inputs_exp = tf.cast(inputs, dtype=tf.float32)
        encoder_output = self.encoder_obj(inputs_exp, training=training, mask=mask)
