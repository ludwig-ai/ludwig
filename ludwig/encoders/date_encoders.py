...
class DateEmbed(DateEncoder):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        scaled_year = self.year_fc(
            tf.cast(input_vector[:, 0:1], tf.float32), training=training, mask=mask
        )
        embedded_month = self.embed_month(
            input_vector[:, 1] - 1, training=training, mask=mask
        )
        embedded_day = self.embed_day(
            input_vector[:, 2] - 1, training=training, mask=mask
        )
        embedded_weekday = self.embed_weekday(
            input_vector[:, 3], training=training, mask=mask
        )
        embedded_yearday = self.embed_yearday(
            input_vector[:, 4] - 1, training=training, mask=mask
        )
        embedded_hour = self.embed_hour(
            input_vector[:, 5], training=training, mask=mask
        )
        embedded_minute = self.embed_minute(
            input_vector[:, 6], training=training, mask=mask
        )
        embedded_second = self.embed_second(
            input_vector[:, 7], training=training, mask=mask
        )

        periodic_second_of_day = tf.sin(
            tf.cast(input_vector[:, 8:9], dtype=tf.float32) * (2 * math.pi / 86400)
        )
...
class DateWave(DateEncoder):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        scaled_year = self.year_fc(input_vector[:, 0:1], training=training, mask=mask)
        ...
        periodic_second_of_day = tf.sin(input_vector[:, 8:9] * (2 * math.pi / 86400))
