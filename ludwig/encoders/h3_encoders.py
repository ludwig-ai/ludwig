...
class H3Embed(H3Encoder):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        embedded_mode = self.embed_mode(
            input_vector[:, 0:1], training=training, mask=mask
        )
        embedded_edge = self.embed_edge(
            input_vector[:, 1:2], training=training, mask=mask
        )
        embedded_resolution = self.embed_resolution(
            input_vector[:, 2:3], training=training, mask=mask
        )
        embedded_base_cell = self.embed_base_cell(
            input_vector[:, 3:4], training=training, mask=mask
        )
        embedded_cells = self.embed_cells(
            input_vector[:, 4:], training=training, mask=mask
        )

        # ================ Masking ================
        resolution = input_vector[:, 2]
        mask = tf.cast(
            tf.expand_dims(tf.sequence_mask(resolution, 15), -1), dtype=tf.float32
        )
...
class H3WeightedSum(H3Encoder):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # ================ Embeddings ================
        input_vector = inputs
        embedded_h3 = self.h3_embed(input_vector, training=training, mask=mask)
...
class H3RNN(H3Encoder):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        # ================ Embeddings ================
        embedded_h3 = self.h3_embed(inputs, training=training, mask=mask)

        # ================ RNN ================
        hidden, final_state = self.recurrent_stack(
            embedded_h3["encoder_output"], training=training, mask=mask
        )
