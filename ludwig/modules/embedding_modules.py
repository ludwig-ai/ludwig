...
def embedding_matrix(
    ...
    if representation == "dense":
        ...
        embeddings = tf.Variable(
            embedding_initializer_obj, trainable=embeddings_trainable, name="embeddings"
        )
...
class EmbedSparse(Layer):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        sparse_multiple_hot_indexes = tf.SparseTensor(
            idx, idx[:, 1], tf.shape(inputs, out_type=tf.int64)
        )
...
class EmbedSequence(Layer):
    ...
    def call(self, inputs, training=None, mask=None):
        ...
        if mask is not None:
            mask_matrix = tf.cast(tf.expand_dims(mask, -1), dtype=tf.float32)
...
class TokenAndPositionEmbedding(Layer):
    def __init__(
        ...
        self.position_embed = Embedding(
            input_dim=max_length, output_dim=self.token_embed.embedding_size
        )
