...
def ClippedOptimizer(
    ...
    **kwargs
    ...
    return clip_optimizer(
        optimizer, clipglobalnorm, clipnorm, clipvalue, horovod=horovod
    )


def clip_optimizer(optimizer, clipglobalnorm, clipnorm, clipvalue, horovod=None):
    class _ClippedOptimizer(tf.keras.optimizers.Optimizer):
        ...
        def minimize_with_tape(self, tape, loss, variables):
            ...
            if self.clipglobalnorm:
                gradients, _ = tf.clip_by_global_norm(gradients, self.clipglobalnorm)
            if self.clipnorm:
                gradients = map(lambda x: tf.clip_by_norm(x, self.clipnorm), gradients)
            if self.clipvalue:
                gradients = map(
