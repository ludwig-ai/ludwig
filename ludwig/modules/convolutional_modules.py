...
class Conv1DLayer(Layer):
    def __init__(
        ...
        if pool_size is not None:
            ...
            self.layers.append(
                pool(pool_size=pool_size, strides=pool_strides, padding=pool_padding)
            )
...
class Conv2DLayerFixedPadding(Layer):
    def __init__(
        self, num_filters=256, filter_size=3, strides=1, weights_regularizer=None
    ):
        super().__init__()
...
class ResNet(Layer):
    def __init__(
        ...
        if first_pool_size:
            self.layers.append(
                MaxPool2D(
                    pool_size=first_pool_size, strides=first_pool_stride, padding="same"
                )
            )
