...
class SequenceReducer(Layer):
    def __init__(self, reduce_mode=None):
        ...
        self._reduce_obj = get_from_registry(reduce_mode, reduce_mode_registry)()
...
class ReduceConcat(Layer):
    ...
    def call(self, inputs, training=None, mask=None):
        if inputs.shape.as_list()[-2] is None or inputs.shape.as_list()[-1] is None:
            ...
            return tf.reshape(inputs, [-1, inputs.shape[-2] * inputs.shape[-1]])
