...
def test_download_mnist_dataset():
    with tempfile.TemporaryDirectory() as source_dir:
        train_image_archive_filename = os.path.join(
            source_dir, "train-images-idx3-ubyte"
        )
        ...
        train_labels_archive_filename = os.path.join(
            source_dir, "train-labels-idx1-ubyte"
        )
        ...
        test_labels_archive_filename = os.path.join(
            source_dir, "t10k-labels-idx1-ubyte"
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            with mock.patch(
                "ludwig.datasets.base_dataset.read_config", return_value=config
            ):
