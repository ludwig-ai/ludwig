...
def test_gpt_encoder(use_pretrained: bool, reduce_output: str, max_sequence_length: int):
    # Use small model params to avoid slow initialization (default n_positions=40478 is huge)
...
@pytest.mark.parametrize("max_sequence_length", [20])
@pytest.mark.skipif(
    not hasattr(__import__("transformers"), "TransfoXLModel"),
    reason="TransfoXL removed from transformers 5.x",
)
def test_transfoxl_encoder(use_pretrained: bool, reduce_output: str, max_sequence_length: int):
    # Use small model params to avoid slow initialization (default vocab_size=267735 is huge)
...
def test_ctrl_encoder(use_pretrained: bool, reduce_output: str, max_sequence_length: int):
    # Use small model params to avoid slow initialization (default vocab_size=246534 is huge)
