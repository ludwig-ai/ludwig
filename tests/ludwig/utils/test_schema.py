...
from ludwig.utils.schema import OUTPUT_FEATURE_TYPES, validate_config
...
def test_config_bad_combiner():
    ...
    with pytest.raises(
        ValidationError, match=r"^\[\{'type': 'tabnet'\}\] is not of .*"
    ):
        validate_config(config)
...
def test_config_fill_values():
    ...
    for vector_fill_value, binary_fill_value in zip(
        vector_fill_values, binary_fill_values
    ):
        config = {
            "input_features": [
...
def test_validate_with_preprocessing_defaults():
    config = {
        "input_features": [
            ...
            numerical_feature(
                preprocessing=NumericalFeatureMixin.preprocessing_defaults
            ),
