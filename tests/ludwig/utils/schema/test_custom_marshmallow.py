...
def test_InitializerOptions():
    ...
    assert set(default_initializer_options.validate.validators[1].choices) == initializer_registry_set
    ...
    assert set(initializer_options.validate.validators[1].choices) == initializer_registry_set

    # Test invalid default case:
    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.InitializerOptions("test"))
...
def test_ReductionOptions():
    ...
    assert set(default_reduction_options.validate.validators[1].choices) == set(list(reduce_mode_registry) + [None])
    ...
    assert set(reduction_options.validate.validators[1].choices) == set(list(reduce_mode_registry) + [None])

    # Test invalid default case:
    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.ReductionOptions("dog"))
...
def test_RegularizerOptions():
    ...
    assert set(default_regularizer_options.validate.validators[1].choices) == {"l1", "l2", "l1_l2", None}
    ...
    assert set(regularizer_options.validate.validators[1].choices) == {"l1", "l2", "l1_l2"}

    # Test invalid default case:
    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.RegularizerOptions("dog"))
...
def test_StringOptions():
    test_options = []
    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.StringOptions(test_options))

    test_options = ["one"]
    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.StringOptions(test_options, default="two"))

    test_options = ["one"]
    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.StringOptions(test_options, default=None, nullable=False))
    ...
    assert string_options.validate.validators[1].choices == ["one", None]
...
def test_PositiveInteger():
    ...
    assert default_positive_integer.validate.validators[1].min == 1

    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.PositiveInteger(-1))

    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.PositiveInteger(1.0))

    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.PositiveInteger(1.1))
    ...
    assert positive_integer.validate.validators[1].min == 1
...
def test_NonNegativeInteger():
    ...
    assert default_nonnegative_integer.validate.validators[1].min == 0

    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.NonNegativeInteger(-1))

    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.NonNegativeInteger(0.0))

    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.NonNegativeInteger(0.1))
    ...
    assert nonnegative_integer.validate.validators[1].min == 0
...
def test_IntegerRange():
    ...
    assert default_integer_range.validate.validators[1].min is None
    assert default_integer_range.validate.validators[1].max is None
    assert default_integer_range.validate.validators[1].min_inclusive is True
    assert default_integer_range.validate.validators[1].max_inclusive is True

    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.IntegerRange(0.0))

    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.IntegerRange(0.1))
    ...
    assert integer_range.validate.validators[1].min is None
    assert integer_range.validate.validators[1].max is None
    assert integer_range.validate.validators[1].min_inclusive is True
    assert integer_range.validate.validators[1].max_inclusive is True

    integer_range = get_marshmallow_from_dataclass_field(lusutils.IntegerRange(0, min=-1, max=1))
    assert integer_range.default == 0
    assert integer_range.allow_none is False
    assert integer_range.validate.validators[1].min == -1
    assert integer_range.validate.validators[1].max == 1
    assert integer_range.validate.validators[1].min_inclusive is True
    assert integer_range.validate.validators[1].max_inclusive is True

    with pytest.raises(MarshmallowValidationError):
        get_marshmallow_from_dataclass_field(lusutils.IntegerRange(0, min=0, max=1, min_inclusive=False))
