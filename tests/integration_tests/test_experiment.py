...
def test_experiment_multiclass_with_class_weights(csv_filename):
    ...
    output_features = [
        category_feature(vocab_size=3, loss={"class_weights": [0, 1, 2, 3]})
    ]
...
def test_experiment_multilabel_with_class_weights(csv_filename):
    ...
    input_features = [category_feature(vocab_size=10)]
    output_features = [
        set_feature(vocab_size=3, loss={"class_weights": [0, 0, 1, 2, 3]})
    ]
...
def test_basic_image_feature(
    num_channels, image_source, in_memory, skip_save_processed_input, csv_filename
):
    # Image Inputs
    ...
    if image_source == "file":
        ...
        df[image_feature_name] = df[image_feature_name].apply(lambda x: imread(x))
...
ImageParms = namedtuple(
    "ImageTestParms", "image_encoder in_memory_flag skip_save_processed_input"
)
...
def test_experiment_image_dataset(
    ...
    model.train(
        dataset=train_dataset_to_use, training_set_metadata=training_set_metadata
    )
...
def test_experiment_tied_weights(csv_filename):
    ...
    input_features = [
        text_feature(
            name="text_feature1", min_len=1, encoder="cnnrnn", reduce_output="sum"
        ),
        text_feature(
            name="text_feature2",
            min_len=1,
...
def test_sequence_tagger(enc_cell_type, attention, csv_filename):
    ...
    input_features = [
        sequence_feature(
            max_len=10, encoder="rnn", cell_type=enc_cell_type, reduce_output=None
        )
    ]
    output_features = [
        sequence_feature(
            max_len=10, decoder="tagger", attention=attention, reduce_input=None
        )
    ]
...
def test_experiment_sequence_combiner(sequence_combiner_encoder, csv_filename):
    ...
    for encoder in ENCODERS[:-2]:
        logger.error("sequence combiner. encoders: {}, {}".format(encoder, encoder))
        input_features[0]["encoder"] = encoder
        input_features[1]["encoder"] = encoder
...
def test_experiment_sampled_softmax(csv_filename):
    ...
    output_features = [
        category_feature(vocab_size=500, loss={"type": "sampled_softmax_cross_entropy"})
    ]

    # Generate test data
    rel_path = generate_data(
        input_features, output_features, csv_filename, num_examples=10000
    )
