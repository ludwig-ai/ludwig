...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            [image_feature(IMAGE_DIR, encoder='stacked_cnn')],
            [numerical_feature()]
        ),
        (
            [image_feature(IMAGE_DIR, encoder='resnet')],
            [category_feature()]
        ),
        (
...
def test_regularizers(
    ...
    for regularization_type in [None, 'l1', 'l2', 'l1_l2']:
        ...
        processed_data_df, _, _, _ = preprocess_for_training(
            config, data_df, backend=backend
        )
    ...
    assert torch.isclose(
        regularizer_losses[1] + regularizer_losses[2] - regularizer_losses[0],
        regularizer_losses[3],
        rtol=0.1
    )
