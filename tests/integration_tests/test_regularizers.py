...
def test_encoder(test_case):
    ...
    for regularizer in [None, "l1", "l2", "l1_l2"]:
        ...
        x_coder_kwargs = dict(
            zip(
                test_case.regularizer_parm_names,
        ...
        if features[0]["type"] in SEQUENCE_TYPES.union({"category", "set"}):
            ...
            training_set.dataset[proc_column] = training_set.dataset[
                proc_column
            ].astype(np.int32)
...
def test_decoder(test_case):
    ...
    combiner_outputs_rank2 = {
        "combiner_output": tf.random.normal([BATCH_SIZE, HIDDEN_SIZE], dtype=tf.float32)
    }

    combiner_outputs_rank3 = {
        "combiner_output": tf.random.normal(
            [BATCH_SIZE, SEQ_SIZE, HIDDEN_SIZE], dtype=tf.float32
        ),
        "encoder_output_state": tf.random.normal(
            [BATCH_SIZE, HIDDEN_SIZE], dtype=tf.float32
        ),
        "lengths": tf.convert_to_tensor(
            np.array(BATCH_SIZE * [SEQ_SIZE]), dtype=tf.int32
        ),
    }
    ...
    training_set, _, _, training_set_metadata = preprocess_for_training(
        config, training_set=df, skip_save_processed_input=True, random_seed=RANDOM_SEED
    )
    ...
    for regularizer in [None, "l1", "l2", "l1_l2"]:
        ...
        if features[0]["type"] in SEQUENCE_TYPES:
            features[0]["num_classes"] = (
                training_set_metadata[feature_name]["vocab_size"] + 1
            )
            training_set.dataset[proc_column] = training_set.dataset[
                proc_column
            ].astype(np.int32)
        ...
        output_def_obj(((combiner_outputs, None), targets), training=True, mask=None)
