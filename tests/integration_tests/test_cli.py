...
def _run_commands(commands, **ludwig_kwargs):
    ...
    completed_process = subprocess.run(
        cmdline, shell=True, stdout=subprocess.PIPE, env=os.environ.copy()
    )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with StringIO(substring) as f:
            for _, line in enumerate(f):
                if not (line[:6] == "Layers" or line[:6] == "Weight") and len(line) > 1:
                    layers_list.append(line[:-1])

        # search for substring with weights names
        weights = re.search("Weights(\\w|\\d|\\:|\\/|\n)*", stdout)
