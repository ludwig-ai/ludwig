...
# def test_early_stopping_nan_weights(tmpdir, caplog):
#     input_features = [text_feature()]
#     output_features = [category_feature(decoder={"vocab_size": 2}, reduce_input="sum")]

#     csv_filename = os.path.join(tmpdir, "training.csv")
#     data_csv = generate_data(input_features, output_features, csv_filename, num_examples=24)
#     val_csv = shutil.copyfile(data_csv, os.path.join(tmpdir, "validation.csv"))
#     test_csv = shutil.copyfile(data_csv, os.path.join(tmpdir, "test.csv"))
...
#     #         if progress_tracker.epoch == 2:
#     #             raise RuntimeError("Interrupting training")

#     model = LudwigModel.load(os.path.join(tmpdir, "api_experiment_run", "model"))  # callbacks=[InterruptCallback()])
#     model.config_obj.trainer.epochs = 3

#     # Manually set the embedding layer weights to nan 2 steps into training
#     input_feature_name = model.config_obj.input_features[0].name + "__ludwig"
#     shape = model.model.input_features.module_dict[
#         input_feature_name
#     ].encoder_obj.embed_sequence.embeddings.weight.shape
#     model.model.input_features.module_dict[
#         input_feature_name
#     ].encoder_obj.embed_sequence.embeddings.weight.data = torch.full(shape, float("nan"))

#     # Resume training
#     # TODO: Current issue is that on resumption the weights are reloaded so the above manual setting of the weights
#     # is lost. Need to find a way to prevent this.
#     model.train(
#         training_set=data_csv,
#         validation_set=val_csv,
#         test_set=test_csv,
#         # model_resume_path=os.path.join(tmpdir, "api_experiment_run"),
#     )

#     assert "NaN or inf tensors found in the model. Stopping training." in caplog.text
