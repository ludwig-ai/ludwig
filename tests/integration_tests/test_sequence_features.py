...
def generate_sequence_training_data():
    input_features = [
        sequence_feature(
            min_len=5, max_len=10, encoder="rnn", cell_type="lstm", reduce_output=None
        )
    ]
    ...
    dataset = build_synthetic_dataset(
        150, copy.deepcopy(input_features) + copy.deepcopy(output_features)
    )
...
def test_sequence_encoders(enc_encoder, enc_cell_type, generate_sequence_training_data):
    ...
    with setup_model_scaffolding(raw_df, input_features, output_features) as (
        model,
        batcher,
    ):
        ...
        while not batcher.last_batch():
            ...
            proc_column = model.model.input_features[input_feature_name].proc_column
            ...
            if enc_encoder == "parallel_cnn":
                number_parallel_cnn_layers = len(
                    model.model.input_features[
                        input_feature_name
                    ].encoder_obj.conv_layers
                )
                hidden_size = (
                    input_features[0]["state_size"] * number_parallel_cnn_layers
                )
                ...
                number_parallel_cnn_layers = len(
                    model.model.input_features[
                        input_feature_name
                    ].encoder_obj.parallel_conv1d_stack.stacked_parallel_layers[0]
                )
                hidden_size = (
                    input_features[0]["state_size"] * number_parallel_cnn_layers
                )
                ...
                raise ValueError(
                    "{} is an invalid encoder specification".format(enc_encoder)
                )
    ...
    with setup_model_scaffolding(raw_df, input_features, output_features) as (model, _):
        ...
        if combiner_output_shapes[1] is not None:
            if len(combiner_output_shapes[1]) > 1:
                ...
                encoder_output_state = tf.random.normal(combiner_output_shapes[1][0])
