...
@pytest.mark.parametrize(
    ...
    [[BATCH_SIZE, OTHER_HIDDEN_SIZE], [BATCH_SIZE, SEQ_SIZE, OTHER_HIDDEN_SIZE]],
)
@pytest.mark.parametrize(
    "hidden_shape", [[BATCH_SIZE, HIDDEN_SIZE], [BATCH_SIZE, SEQ_SIZE, HIDDEN_SIZE]]
)
@pytest.mark.parametrize(
    "reduce_dependencies", ["sum", "mean", "avg", "max", "concat", "last", "attention"]
)
def test_multiple_dependencies(
    reduce_dependencies, hidden_shape, dependent_hidden_shape, dependent_hidden_shape2
):
    # setup at least for a single dependency
    hidden_layer = tf.random.normal(hidden_shape, dtype=tf.float32)
    other_hidden_layer = tf.random.normal(dependent_hidden_shape, dtype=tf.float32)
    other_dependencies = {
    ...
    if (
        ...
        and len(hidden_shape) == 2
        and len(dependent_hidden_shape) == 3
    ):
    ...
    if dependent_hidden_shape2:
        other_hidden_layer2 = tf.random.normal(
            dependent_hidden_shape2, dtype=tf.float32
        )
        ...
        if (
            reduce_dependencies == "concat"
            and len(hidden_shape) == 2
            and len(dependent_hidden_shape2) == 3
        ):
    ...
    if len(hidden_shape) > 2:
        assert results.shape.as_list() == [BATCH_SIZE, SEQ_SIZE, expected_hidden_size]
    else:
