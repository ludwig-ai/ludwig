...
def run_api_experiment(
    ...
    # Sanity check: auto-detected trainer kwargs should not use GPUs unless cluster has them
    auto_kwargs = get_trainer_kwargs()
    num_gpus = int(ray.cluster_resources().get("GPU", 0))
    if num_gpus > 0:
        assert auto_kwargs.get("use_gpu"), auto_kwargs
    else:
        assert not auto_kwargs.get("use_gpu"), auto_kwargs
...
def run_split_api_experiment(config, dataset=None, backend_config=None, **kwargs):
    train_fname, val_fname, test_fname = split(dataset)
...
def test_ray_read_binary_files(tmpdir, df_engine, ray_cluster_2cpu):
    ...
    # Compare by index alignment since Ray may reorder rows
    proc_col = proc_col.sort_index()
    proc_col_expected = proc_col_expected.sort_index()
    # Drop NaN entries (both backends should agree on non-NaN values)
    proc_col = proc_col.dropna()
    proc_col_expected = proc_col_expected.dropna()
...
@pytest.mark.distributed
def test_ray_tabular_save_inputs(ray_cluster_2cpu):
    input_features = [
        category_feature(encoder={"vocab_size": 2}, reduce_input="sum"),
        number_feature(normalization="zscore"),
        set_feature(),
...
@pytest.mark.distributed
def test_ray_save_outputs(ray_cluster_2cpu):
    input_features = [
        binary_feature(),
    ]
    output_features = [
...
@pytest.mark.distributed
def test_ray_text_sequence_timeseries(ray_cluster_2cpu):
    input_features = [
        text_feature(),
...
@pytest.mark.distributed
def test_ray_vector(ray_cluster_2cpu):
    input_features = [
        vector_feature(),
    ]
    output_features = [
...
@pytest.mark.distributed
def test_ray_audio(tmpdir, ray_cluster_2cpu):
    preprocessing_params = {
        "audio_file_length_limit_in_s": 3.0,
        "missing_value_strategy": BFILL,
        "in_memory": True,
...
@pytest.mark.parametrize("dataset_type", ["csv", "pandas+numpy_images"])
@pytest.mark.distributed
def test_ray_image(tmpdir, dataset_type, ray_cluster_2cpu):
    ...
    input_features = [
        image_feature(
            ...
            preprocessing={"in_memory": True, "height": 8, "width": 8, "num_channels": 3, "num_processes": 2},
...
@pytest.mark.parametrize(
    "settings",
    [(True, False, "ffill"), (False, True, "bfill")],
    ids=["first_row_none", "last_row_none"],
)
@pytest.mark.distributed
...
def test_ray_lazy_load_image_error(tmpdir, ray_cluster_2cpu):
    ...
    input_features = [
        image_feature(
            ...
            preprocessing={"in_memory": False, "height": 8, "width": 8, "num_channels": 3, "num_processes": 2},
        ),
    ]
    output_features = [
        binary_feature(),
    ...
    input_df = pd.DataFrame(
        {
            ...
            "Label": np.concatenate((np.zeros(90), np.ones(10))),
            "split": np.zeros(100),
        }
    )
...
def test_tune_batch_size_lr_cpu(tmpdir, ray_cluster_2cpu):
    ...
    csv_filename = os.path.join(tmpdir, "dataset.csv")
    dataset_csv = generate_data(config["input_features"], config["output_features"], csv_filename, num_examples=50)
...
def test_ray_distributed_predict(tmpdir, ray_cluster_2cpu):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        backend_config = {**RAY_BACKEND_CONFIG}
        csv_filename = os.path.join(tmpdir, "dataset.csv")
        dataset_csv = generate_data(input_features, output_features, csv_filename, num_examples=25)
        ...
        preds = preds.compute()
        assert preds.iloc[1].name != preds.iloc[10].name
