...
def test_train_with_config(time_budget, test_data, ray_cluster_2cpu, tmpdir):
    ...
    with mock.patch("ludwig.hyperopt.execution.RayTuneExecutor._evaluate_best_model") as mock_fn:
        ...
        if time_budget > 1:
            ...
            # With a 1-second time budget, the trial may not complete.
            # best_model may be None if no trial completed.
            # _evaluate_best_model may or may not be called depending on
            # whether the trial had time to report any training stats.
            assert best_model is None or isinstance(best_model, LudwigModel)
