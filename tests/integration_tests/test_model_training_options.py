...
def test_model_progress_save(
    skip_save_progress, skip_save_model, generated_data, tmp_path
):
    input_features, output_features = get_feature_configs()
    ...
    if skip_save_model:
        ...
        assert len(files) >= 2
        assert os.path.isfile(os.path.join(output_dir, "model", "checkpoint"))
    ...
    if skip_save_progress:
        ...
        assert os.path.isdir(os.path.join(output_dir, "model", "training_checkpoints"))
...
def test_regularization(generated_data, tmp_path):
    ...
    config = {
        ...
        "output_features": output_features,
        "combiner": {"type": "concat"},
        "training": {"epochs": 1, "batch_size": 16, "regularization_lambda": 1},
    }
    ...
    for regularizer in [None, "l1", "l2", "l1_l2"]:
        ...
        config["output_features"][0]["weights_regularizer"] = regularizer
        config["output_features"][0]["bias_regularizer"] = regularizer
        config["output_features"][0]["activity_regularizer"] = regularizer
...
def test_cache_checksum(csv_filename, tmp_path):
    ...
    _, _, train_output_directory1 = model.train(
        dataset=source_dataset, output_directory=output_directory
    )
    first_training_timestamp = os.path.getmtime(cache_fname)

    # conduct second training, should not force recreating hdf5
    model = LudwigModel(config, backend=backend)
    _, _, train_output_directory2 = model.train(
        dataset=source_dataset, output_directory=output_directory
    )
    ...
    model = LudwigModel(config, backend=backend)
    _, _, train_output_directory3 = model.train(
        dataset=source_dataset, output_directory=output_directory
    )
    current_training_timestamp = os.path.getmtime(cache_fname)
    ...
    model = LudwigModel(config, backend=backend)
    _, _, train_output_directory4 = model.train(
        dataset=source_dataset, output_directory=output_directory
    )
    current_training_timestamp = os.path.getmtime(cache_fname)
    ...
    model = LudwigModel(config, backend=backend)
    _, _, train_output_directory5 = model.train(
        dataset=source_dataset, output_directory=output_directory
    )
    current_training_timestamp = os.path.getmtime(cache_fname)
