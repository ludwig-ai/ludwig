...
def test_feature(
    input_test_feature, output_test_feature, output_loss_parameter, csv_filename
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ([category_feature()], [set_feature(vocab_size=5), set_feature(vocab_size=7)]),
        (
            [category_feature()],
...
def test_feature_multiple_outputs(
    input_test_feature, output_test_feature, csv_filename
):
    # Generate test data
    rel_path = generate_data(
        input_test_feature, output_test_feature, csv_filename, 1001
    )
...
def test_category_int_dtype(tmpdir):
    ...
    csv_fname = generate_data(
        input_features, output_features, os.path.join(tmpdir, "dataset.csv")
    )
