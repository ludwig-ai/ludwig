...
def test_validation_metrics(test_case: TestCase, csv_filename: str):
    ...
    training_data = generate_data(
        input_features, output_features, filename=csv_filename
    )
...
@pytest.mark.parametrize(
    "test_case",
    [
        TestCase([numerical_feature(), numerical_feature()], []),
        TestCase([category_feature(), numerical_feature()], []),
    ],
)
...
@pytest.mark.parametrize(
    "test_case", [TestCase([numerical_feature()], ["invalid_metric"])]
)
