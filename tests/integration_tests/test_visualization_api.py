...
class Experiment:
    ...
    def __init__(self, csv_filename, tmpdir):
        ...
        data_csv = generate_data(
            self.input_features, self.output_features, self.csv_file
        )
...
def test_learning_curves_vis_api(experiment_to_use):
    ...
    with TemporaryDirectory() as tmpvizdir:
        for viz_output in viz_outputs:
            vis_output_pattern_pdf = tmpvizdir + "/*.{}".format(viz_output)
...
def test_confidence_thresholding_2thresholds_2d_vis_api(csv_filename):
    ...
    with TemporaryDirectory() as tmpvizdir:
        # Generate test data
        data_csv = generate_data(
            input_features, output_features, os.path.join(tmpvizdir, csv_filename)
        )
        ...
        test_stats, predictions, _ = model.evaluate(
            dataset=test_df, collect_predictions=True, output_dir=output_dir
        )
...
def test_roc_curves_from_test_statistics_vis_api(csv_filename):
    ...
    with TemporaryDirectory() as tmpvizdir:
        ...
        data_df = read_csv(data_csv)
        _, _, output_dir = model.train(
            dataset=data_df, output_directory=os.path.join(tmpvizdir, "results")
        )
        # extract test metrics
        test_stats, _, _ = model.evaluate(
            dataset=data_df, collect_overall_stats=True, output_directory=output_dir
        )
        test_stats = test_stats
        viz_outputs = ("pdf", "png")
        for viz_output in viz_outputs:
