...
InputFeatureOptions = namedtuple(
    "InputFeatureOptions", "feature_type feature_options tie_features"
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        InputFeatureOptions(
            "numerical", {"preprocessing": {"normalization": "zscore"}}, True
        ),
        ...
        InputFeatureOptions(
            "audio",
            {"embedding_size": 64, "max_sequence_length": 16, "should_embed": False},
            True,
        ),
        # do not tie input features, encoders should be different
        InputFeatureOptions("numerical", None, False),
        InputFeatureOptions(
            "numerical", {"preprocessing": {"normalization": "zscore"}}, False
        ),
...
def test_tied_micro_level(input_feature_options):
    ...
    input_feature_configs.append(
        {"name": "input_feature_2", "type": input_feature_options.feature_type}
    )
    if input_feature_options.feature_options is not None:
    ...
    if input_feature_options.tie_features:
        ...
        assert (
            input_features["input_feature_1"].encoder_obj
            is input_features["input_feature_2"].encoder_obj
        )
    else:
        # no tied parameter, encoders should be different
        assert (
            input_features["input_feature_1"].encoder_obj
            is not input_features["input_feature_2"].encoder_obj
        )
